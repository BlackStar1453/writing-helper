...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[Tauri Refresh User API] 刷新用户信息...');

    // 验证JWT令牌
    let decoded;
    try {
      decoded = verify(token, JWT_SECRET) as { userId: string; email: string; exp: number };
      console.log('[Tauri Refresh User API] 令牌验证成功:', decoded.userId);
    } catch (error) {
      console.log('[Tauri Refresh User API] 令牌验证失败:', error);
      return NextResponse.json({
        success: false,
        error: '令牌无效或已过期'
      }, { status: 401 });
    ...
    if (userRecord.length === 0) {
      console.log('[Tauri Refresh User API] 用户不存在:', decoded.userId);
      return NextResponse.json({
        success: false,
        error: '用户不存在'
      }, { status: 404 });
    ...
    console.log('[Tauri Refresh User API] 获取用户信息成功:', {
      id: user.id,
      email: user.email,
      plan: user.planName,
    ...
    console.log('[Tauri Refresh User API] 用户信息刷新成功');
    console.log('[Tauri Refresh User API] 响应预览:', {
      success: true,
      user_data: userData
    });
